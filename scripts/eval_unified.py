"""Evaluate the unified hierarchical diffusion model on a Q&A dataset.
...
Usage (single sample):
    python scripts/eval_unified.py \\
        ...
        --unified_model checkpoints/unified_lora

Usage (best-of-4):
    python scripts/eval_unified.py ... --num_samples 4
...
def parse_args():
    ...
    p.add_argument("--unified_model", default="checkpoints/unified_lora",
                   help="Path to trained unified model directory")
...
def main():
    ...
    # Load hierarchy head and embedding
    hier_ckpt = torch.load(
        os.path.join(args.unified_model, "hierarchy_components.pt"),
        map_location="cpu"
    )
    model.hierarchy_head.load_state_dict(hier_ckpt["hierarchy_head"])
    model.hierarchy_embedding.load_state_dict(hier_ckpt["hierarchy_embedding"])
